...
- `new125+kure+langchain.py`
  - Chroma DB, KURE dense retrieval, BM25 sparse retrieval, alias n-gram rescue, metadata reranking을 함께 사용하는 adaptive hybrid retrieval 파이프라인입니다.
...
- `new125+kure+langgraph.py`
  - adaptive hybrid retrieval 구조를 LangGraph 기반 workflow로 확장한 버전입니다.
...
3. KURE query embedding 기반 dense retrieval과 BM25 기반 sparse retrieval을 각각 수행합니다.
4. 질문 유형과 검색 결과에 따라 alias n-gram rescue, fact backfill, detail chunk 확장 등을 보조적으로 수행합니다.
5. dense/sparse/보조 검색 후보를 RRF와 metadata reranking으로 병합하고 재정렬합니다.
6. LLM 입력용 context를 선택합니다.
7. 한국어 instruction-tuned LLM(sh2orc/Llama-3.1-Korean-8B-Instruct) 으로 답변을 생성합니다.
