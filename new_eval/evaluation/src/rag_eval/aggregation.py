...
from .config import PHASE1_METRIC_COLUMNS
...
def aggregate_phase1(phase1_df: pd.DataFrame) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame]:
    ...
    by_type = phase1_df.groupby("type", dropna=False)[list(PHASE1_METRIC_COLUMNS)].mean().reset_index()
    by_difficulty = phase1_df.groupby("difficulty", dropna=False)[list(PHASE1_METRIC_COLUMNS)].mean().reset_index()
